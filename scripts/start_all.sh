...
cd "$ROOT"
python3 "$ROOT/scripts/setup_dependencies.py"
...
start_one() {
  ...
  if [[ -f "$pidfile" ]] && kill -0 "$(cat "$pidfile")" 2>/dev/null; then
    echo "[VALIDATION] $name already running (PID $(cat "$pidfile"))"
    return
  fi
  nohup env VALIDATION_HOME="$ROOT" PYTHONPATH="$PYTHONPATH" "$PY" "$@" >>"$log" 2>&1 < /dev/null &
  echo $! > "$pidfile"
  echo "[VALIDATION] $name started (PID $!)"
}
