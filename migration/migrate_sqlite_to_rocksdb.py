    ...
    try:
        ...
        for table in tables:
            ...
            if database_name=="router_state" and table=="file_states":
                ...
                grouped={}
                for r in rows:
                    grouped.setdefault(int(r["mmsi"]),[]).append({"mmsi":int(r["mmsi"]),"message_type":int(r["message_type"]),"timestamp":r.get("timestamp"),"source":r.get("source"),"payload":_json(r.get("payload_json")),"updated_at":r.get("updated_at")})
