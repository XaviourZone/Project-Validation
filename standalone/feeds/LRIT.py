...
# ================= OPERATOR SETTINGS =================
SOURCE="LRIT"; SOURCE_ID=4
INPUT_MODE="FOLDER"; INPUT_FOLDER=r"./input/LRIT"; FILE_PATTERNS=["*.csv","*.txt","*.nmea","*.log"]
POLL_SECONDS=2; RUN_ONCE=False
TCP_HOST="127.0.0.1"; TCP_PORT=20001
OUTPUT_FOLDER=r"./output/LRIT"
...
logging.basicConfig(level=logging.INFO,format="%(asctime)s | %(levelname)s | LRIT | %(message)s")
...
def parse_payload(text):
    out=[]
    for i,c in enumerate(csv.reader(io.StringIO(text)),1):
        if not c or (i==1 and c[0].strip().lower()=="mmsi"):continue
        if len(c)<13:log.warning("LRIT line %d rejected: fewer than 13 columns",i);continue
        try:
            out.append({"mmsi":num(c[0],True),"latitude":num(c[1]),"longitude":num(c[2]),"sog":num(c[3]),"cog":num(c[4]),"heading":num(c[5]),
            "nav_status":num(c[7],True),"timestamp":clean(c[8]),"vessel_name":clean(c[11]),"imo":num(c[12],True),
            "callsign":clean(c[13]) if len(c)>13 else None,"length":num(c[14]) if len(c)>14 else None,"width":num(c[15]) if len(c)>15 else None,
            "draft":num(c[16]) if len(c)>16 else None,"destination":clean(c[17]) if len(c)>17 else None,
            "vessel_type":clean(c[18]) if len(c)>18 else None,"eta":clean(c[19]) if len(c)>19 else None})
        except Exception as e:log.warning("LRIT line %d rejected: %s",i,e)
    return out
